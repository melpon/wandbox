...
template <typename KeyT, typename ValueT, typename Hash, typename Pred,
          typename Alloc, typename ActualKey>
boost::optional<const ValueT&> map_find(
    const std::unordered_map<KeyT, ValueT, Hash, Pred, Alloc>& m,
    ActualKey&& k) {
  const auto ite = m.find(std::forward<ActualKey>(k));
  if (ite == m.end()) return boost::none;
  return ite->second;
}
template <typename KeyT, typename ValueT, typename Compare, typename Alloc,
          typename ActualKey>
boost::optional<const ValueT&> map_find(
    const std::map<KeyT, ValueT, Compare, Alloc>& m, ActualKey&& k) {
  const auto ite = m.find(std::forward<ActualKey>(k));
  if (ite == m.end()) return boost::none;
  return ite->second;
}
...
boost::optional<const typename Map::value_type&> map_multi_index_find(
    const Map& m, KeyT&& k) {
...
struct counting_semaphore {
  counting_semaphore(asio::io_service& aio, unsigned count)
      : aio(aio), des(std::make_shared<asio::posix::stream_descriptor>(aio)) {
  ...
  struct semaphore_object {
    template <typename F>
    semaphore_object(asio::io_service& aio,
                     const std::shared_ptr<asio::posix::stream_descriptor>& des,
                     F&& f)
        : aio(aio), des(des) {
      std::clog << "[" << (void*)this << "]"
                << " semaphore create" << std::endl;
      const auto b = std::make_shared<std::array<unsigned char, 8>>();
      asio::async_read(*des, asio::buffer(*b),
                       std::bind<void>(
                           [](F f, error_code ec, std::shared_ptr<void>) {
                             if (!ec) f();
                           },
                           std::forward<F>(f), _1, b));
    }
...
struct socket_write_buffer : std::enable_shared_from_this<socket_write_buffer> {
  ...
  void async_write_command(std::string cmd, std::string data,
                           Handler&& handler) {
    std::unique_lock<std::recursive_mutex> l(mtx);
    data = quoted_printable::encode(move(data));
    back_buf.emplace_back(cmd + " " + std::to_string(data.length()) + ":" +
                          data + "\n");
  ...
  void flush() {
    ...
    if (!writing) {
      ...
      async_write(*sock, b,
                  std::bind<void>(std::mem_fn(&socket_write_buffer::on_wrote),
                                  shared_from_this()));
      back_buf = {};
      back_handlers = {};
      writing = true;
...
struct program_runner : private coroutine {
  ...
  struct status_forwarder : pipe_forwarder_base {
    status_forwarder(std::shared_ptr<asio::io_service> aio,
                     std::shared_ptr<asio::signal_set> sigs,
                     unique_child_pid&& pid)
        : aio(move(aio)), sigs(move(sigs)), pid(move(pid)) {}
    bool closed() const noexcept override { return pid.finished(); }
    void async_forward(std::function<void()> handler) noexcept override {
      sigs->async_wait(std::bind<void>(&status_forwarder::wait_handler,
                                       ref(*this), handler));
    }
    int get_status() noexcept { return pid.wait_nonblock(); }
  ...
  struct input_forwarder : pipe_forwarder_base {
    input_forwarder(std::shared_ptr<asio::io_service> aio, unique_fd&& fd,
                    std::string input)
        : aio(move(aio)), pipe(*this->aio), input(move(input)) {
      pipe.assign(fd.get());
      fd.release();
    }
    bool closed() const noexcept override { return !pipe.is_open(); }
    void async_forward(std::function<void()> handler) noexcept override {
      async_write(
          pipe, asio::buffer(input),
          std::bind<void>(&input_forwarder::on_wrote, ref(*this), handler));
    }
  ...
  struct write_limit_counter {
    explicit write_limit_counter(size_t soft_limit, size_t hard_limit)
        : soft_limit(soft_limit), hard_limit(hard_limit), current(0), proc() {}
    void set_process(std::shared_ptr<status_forwarder> proc) {
      this->proc = move(proc);
    }
    void add(size_t len) {
      if (std::numeric_limits<size_t>::max() - len < current)
  ...
  struct output_forwarder : pipe_forwarder_base, private coroutine {
    output_forwarder(std::shared_ptr<asio::io_service> aio,
                     std::shared_ptr<tcp::socket> sock, unique_fd&& fd,
                     std::string command,
  ...
  program_runner(std::shared_ptr<asio::io_service> aio,
                 std::shared_ptr<tcp::socket> sock,
                 std::unordered_map<std::string, std::string> received,
                 std::shared_ptr<asio::signal_set> sigs,
                 std::shared_ptr<DIR> workdir, compiler_trait target_compiler,
      ...
      : aio(move(aio)),
        ...
        limitter(std::make_shared<write_limit_counter>(jail.output_limit_warn,
  ...
  void operator()(error_code ec = error_code(), size_t = 0) {
    reenter(this) {
      std::clog << "[" << sock.get() << "]"
                << "running program with '" << target_compiler.name << "' ["
                << this << "]" << std::endl;
      {
        namespace qi = boost::spirit::qi;
        ...
        if (const auto r = map_find(received, "CompilerOption")) {
          ...
          for (const auto& sw : selected_switches) {
            auto t =
                map_multi_index_find<1>(target_compiler.local_switches, sw);
        ...
        for (auto& x : {std::make_pair("CompilerOptionRaw", &ccargs),
                        std::make_pair("RuntimeOptionRaw", &progargs)}) {
        ...
        progargs.insert(progargs.begin(), jail.jail_command.begin(),
                        jail.jail_command.end());
        commands = {{move(ccargs), "", "CompilerMessageS", "CompilerMessageE",
                     jail.compile_time_limit},
                    {move(progargs), "StdIn", "StdOut", "StdErr",
                     jail.program_duration}};
      }

      yield {
        PROTECT_FROM_MOVE(strand);
        PROTECT_FROM_MOVE(sockbuf);
        sockbuf->async_write_command("Control", "Start",
                                     strand->wrap(move(*this)));
      }
      ...
      while (!commands.empty()) {
        ...
        std::clog << std::endl;
        {
          ...
          pipes = {
              std::make_shared<input_forwarder>(
                  aio, move(c.fd_stdin), received[current.stdin_command]),
              std::make_shared<output_forwarder>(aio, sock, move(c.fd_stdout),
                                                 current.stdout_command,
                                                 limitter),
              std::make_shared<output_forwarder>(aio, sock, move(c.fd_stderr),
                                                 current.stderr_command,
                                                 limitter),
              std::make_shared<status_forwarder>(aio, sigs, move(c.pid)),
          };
          limitter->set_process(
              std::static_pointer_cast<status_forwarder>(pipes[3]));
        }
      ...
      wait_process_killed:
        ...
        laststatus =
            std::static_pointer_cast<status_forwarder>(pipes[3])->get_status();
        if (!WIFEXITED(laststatus) || (WEXITSTATUS(laststatus) != 0)) break;
      }
      if (WIFEXITED(laststatus)) yield {
          PROTECT_FROM_MOVE(strand);
          PROTECT_FROM_MOVE(sockbuf);
          sockbuf->async_write_command("ExitCode",
                                       std::to_string(WEXITSTATUS(laststatus)),
                                       strand->wrap(move(*this)));
        }
      if (WIFSIGNALED(laststatus)) yield {
          PROTECT_FROM_MOVE(strand);
          PROTECT_FROM_MOVE(sockbuf);
          sockbuf->async_write_command("Signal",
                                       ::strsignal(WTERMSIG(laststatus)),
                                       strand->wrap(move(*this)));
        }
      std::clog << "[" << sock.get() << "]"
      ...
      yield {
        PROTECT_FROM_MOVE(strand);
        PROTECT_FROM_MOVE(sockbuf);
        sockbuf->async_write_command("Control", "Finish",
                                     strand->wrap(move(*this)));
      }
    }
  }
...
struct program_writer : private coroutine {
  typedef void result_type;
  program_writer(std::shared_ptr<asio::io_service> aio,
                 std::shared_ptr<tcp::socket> sock,
                 std::shared_ptr<asio::signal_set> sigs,
                 std::unordered_map<std::string, std::string> received,
    ...
    std::clog << "[" << this->sock.get() << "]"
              << "create log directory '"
              << (config.system.storedir + "/" + s + "/" + unique_name) << "' ["
              << this << "]" << std::endl;
    const auto logdir = mkdir_p_open_at(
        {}, config.system.storedir + "/" + s + "/" + unique_name, 0700);
    if (!logdir) {
      std::clog << "[" << this->sock.get() << "]"
    ...
    std::clog << "[" << this->sock.get() << "]"
              << "using temporary name '" << unique_name << "' [" << this << "]"
              << std::endl;
    const auto savedir = mkdir_p_open_at(workdir, "store", 0700);
    if (!savedir) {
      std::clog << "[" << this->sock.get() << "]"
                << "failed to create working directory '" << unique_name
                << "' [" << this << "]" << std::endl;
      throw_system_error(errno);
    }
    ...
    for (auto&& x : sources) {
      std::clog << "[" << this->sock.get() << "]"
                << "registering file '" << x.first << "' [" << this << "]"
                << std::endl;
      if (x.first.empty()) {
        const source_file_t filebase(this->target_compiler.output_file,
                                     x.second, nullptr);
        ...
        for (size_t n = 0; n < tree.size(); ++n) {
          ...
          std::clog << "[" << this->sock.get() << "]"
                    << "create source subdirectory '" << tree[n] << "' ["
                    << this << "]" << std::endl;
          ...
          for (auto&& p : std::vector<std::shared_ptr<DIR>>(parents.begin(),
                                                            parents.end())) {
            dirs.emplace(tree[n],
                         (mkdirat(p, dirname, 0700), opendirat(p, dirname)));
          }
        }
  ...
  void operator()(error_code = error_code(), size_t = 0) {
    reenter(this) {
      while (!sources.empty()) {
        std::clog << "[" << sock.get() << "]"
                  << "writing file '" << sources.front().name << "' [" << this
                  << "]" << std::endl;

        {
          ::memset(aiocb.get(), 0, sizeof(*aiocb.get()));
          while (true) {
            aiocb->aio_fildes = ::openat(
                dirfd(sources.front().dir.get()),
                ("./" + sources.front().name).c_str(),
                O_WRONLY | O_CLOEXEC | O_CREAT | O_TRUNC | O_EXCL | O_NOATIME,
                0600);
            ...
            if (errno == EAGAIN || errno == EMFILE || errno == EWOULDBLOCK) {
              ...
              std::clog << "[" << sock.get() << "]"
                        << "open failed '" << sources.front().name.c_str()
                        << "' [" << this << "]" << std::endl;
              yield break;
            }
          }
          ...
          std::clog << "[" << sock.get() << "]"
                    << "write success '" << sources.front().name << "' ["
                    << this << "]" << std::endl;
          sources.pop_front();
        }
      }
      return program_runner(aio, move(sock), move(received), move(sigs),
                            move(workdir), move(target_compiler),
                            move(semaphore))();
    }
  }
  std::shared_ptr<asio::io_service> aio;
  ...
  struct source_file_t {
    ...
    source_file_t(std::string filename, const std::string& source,
                  std::shared_ptr<DIR> dir)
...
struct version_sender : private coroutine {
  typedef void result_type;
  version_sender(std::shared_ptr<asio::io_service> aio,
                 std::shared_ptr<tcp::socket> sock,
                 std::shared_ptr<asio::signal_set> sigs,
                 std::shared_ptr<void> semaphore)
      : aio(move(aio)),
  ...
  void operator()(error_code = error_code(), size_t = 0) {
    reenter(this) {
      std::clog << "[" << sock.get() << "]"
                << "building compiler list" << std::endl;
      while (!commands.empty()) {
        current = move(commands.front());
        commands.pop_front();
        if (current.version_command.empty() || not current.displayable)
          continue;
        {
          auto c = piped_spawn(opendir("/"), current.version_command);
          std::clog << "[" << sock.get() << "]"
                    << "run ["
                    << boost::algorithm::join(move(current.version_command),
                                              ", ")
                    << "]" << std::endl;
          child = std::make_shared<unique_child_pid>(move(c.pid));
          pipe_stdout = std::make_shared<asio::posix::stream_descriptor>(
              *aio, c.fd_stdout.get());
          c.fd_stdout.release();
        }
        do {
...

        {
          ...
          std::cout << "[" << sock.get() << "]"
                    << "WIFEXITED(st): " << WIFEXITED(st)
                    << ", WEXITSTATUS(st): " << WEXITSTATUS(st) << std::endl;
...

        {
          ...
          std::cout << "[" << sock.get() << "]"
                    << "add version: " << ver << std::endl;
          versions.emplace_back(generate_displaying_compiler_config(
              move(current), ver, config.switches));
        }
      }
      yield {
        auto s =
            "{"
            "\"compilers\":[" +
            boost::algorithm::join(move(versions), ",") +
            "],"
            "\"templates\":" +
            generate_templates(config.templates) + "}";
        PROTECT_FROM_MOVE(sockbuf);
...
struct compiler_bridge : private coroutine {
  typedef void result_type;
  compiler_bridge(std::shared_ptr<asio::io_service> aio,
                  std::shared_ptr<tcp::socket> sock,
                  std::shared_ptr<asio::signal_set> sigs,
                  std::shared_ptr<void> semaphore)
      : aio(move(aio)),
  ...
  void operator()(error_code ec = error_code(), size_t len = 0) {
    reenter(this) while (true) {
      yield {
        ...
        sock->async_read_some(asio::buffer(asio::buffer(*buf) + offset),
                              move(*this));
      }
      ...
      while (true) {
        ...
        int len = 0;
        std::string data;
        if (!qi::parse(ite, buf->end(),
                       +(qi::char_ - qi::space) >> qi::omit[*qi::space] >>
                           qi::omit[qi::int_[phx::ref(len) = qi::_1]] >>
                           qi::omit[':'] >>
                           qi::repeat(phx::ref(len))[qi::char_] >>
                           qi::omit[qi::eol],
                       command, data))
          break;
        if (command == "Control" && data == "run") {
          ...
          if (c == config.compilers.get<1>().end()) {
            std::clog << "[" << sock.get() << "]"
                      << "selected compiler '" << ccname
                      << "' is not configured" << std::endl;
            return (void)sock->close(ec);
          }
          return program_writer(move(aio), move(sock), move(sigs),
                                move(received), move(sources), *c,
                                move(semaphore))();
        } else if (command == "Version") {
          return version_sender(move(aio), move(sock), move(sigs),
                                move(semaphore))();
...
struct listener : private coroutine {
  ...
  void operator()(error_code = error_code()) {
    reenter(this) while (true) {
      ...
      std::clog << "[" << sock.get() << "]"
                << "connection established from " << sock->remote_endpoint()
                << std::endl;
  ...
  listener(std::shared_ptr<asio::io_service> aio, Args&&... args)
      : aio(move(aio)),
        ...
        sock(),
        sem(std::make_shared<counting_semaphore>(
            *this->aio, config.system.max_connections - 1)) {
